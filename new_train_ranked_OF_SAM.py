...
def finetune_epoch(ft_loader, model, model_ema, optimizer, scheduler, device, best_, epoch=-1,
                   need_feat=False, savemodel=False, date='0709'):
    # suppose batch_size = 1
    print('Save weights in training? {}'.format(savemodel))
    model.train()
    loss_in_epoch = []
    ...
    os.makedirs('./em_weights/train_{}/'.format(date), exist_ok=True)
    import pickle
    with open('./em_weights/train_{}/train_loss.pkl'.format(date), 'wb') as f:
        pickle.dump(loss_in_epoch, f)

    # # 3.91 validation部分出现过爆显存问题，转移到train部分也存储一次
    # if savemodel:
    ...
    #         },
    #         f"em_weights/SAM_boundary_of_ranked.pth",
    #     )
...
def inference_set(inf_loader, model, device, best_, save_model=True, suffix='s',
                  save_name="divide", deform_mode='tps', date='0709'):
    ...
    if val_ranked_loss < best_ranked and save_model:
        ...
        torch.save(
            ...
            f"em_weights/{save_name}_ranked.pth",
        )
    ...
    os.makedirs('./em_weights/val_results/', exist_ok=True)
    import pickle
    with open('./em_weights/val_results/J{}_{}.pkl'.format(date, deform_mode), 'wb') as f:
        pickle.dump(results, f)
    os.makedirs('./em_weights/train_{}/'.format(date), exist_ok=True)
    with open('./em_weights/train_{}/val_loss.pkl'.format(date), 'wb') as f:
        pickle.dump(loss_in_epoch, f)
...
def main():
    # Read In options
    # with open('options/of_SAMguided_3dot91.yml', "r") as f:
    #     opt = yaml.safe_load(f)
    # print(opt)
    # with open('options/of_SAMguided_3dot1.yml', "r") as f:
    #     opt = yaml.safe_load(f)
    # with open('options/of_SAMdivide_3dot1.yml', "r") as f:
    #     opt = yaml.safe_load(f)
    # with open('options/of_SAMdivide_3dot91.yml', "r") as f:
    #     opt = yaml.safe_load(f)
    # with open('options/of_SAMboundary_3dot1.yml', "r") as f:
    #     opt = yaml.safe_load(f)
    # with open('options/of_SAMboundary_3dot91.yml', "r") as f:
    #     opt = yaml.safe_load(f)
    ...
    os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'max_split_size_mb:32'  # 3.91推理部分爆内存尝试解决方案
    ...
    for split in range(num_splits):
        ...
        for epoch in range(opt["num_epochs"]):
            ...
            for key, train_loader in train_loaders.items():
                finetune_epoch(
                    train_loader, model, model_ema, optimizer, scheduler, device, bests_n['val'], epoch,
                    opt.get("need_feat", False), savemodel=True)
