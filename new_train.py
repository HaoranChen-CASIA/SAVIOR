    ...
    for i, datas in enumerate(tqdm(ft_loader, desc=f"Training in epoch {epoch}")):
        ...
        if need_upsampled:
            if need_feat:
                ...
                y_pred_up = model(up_video, inference=False)
                if(y_pred.dim()!=2):
                    y_pred_up = y_pred_up.mean((-3, -2, -1))
...
def inference_set(inf_loader, model, device, best_, save_model=False, suffix='s', save_name="divide"):
    ...
    if s + p > best_s + best_p and save_model:
        ...
        torch.save(
            ...
            f"pretrained_weights/{save_name}_{suffix}_dev_v1.0.pth",
        )
...
def main():
    ...
    # # parser.add_argument(
    # #     "-o", "--opt", type=str, default="options/fast/vs-resize.yml", help="the option file"
    # # )
    # parser.add_argument(
    #     "-o", "--opt", type=str, default="./options/stable.yml", help="the option file"
    # )
    #
    ...
    for split in range(num_splits):
        ...
        for epoch in range(opt["num_epochs"]):
            ...
            for key in val_loaders:
                ...
                #    model_ema if model_ema is not None else model,
                #    device, bests[key], save_model=opt["save_model"], save_name=opt["name"],
                #    suffix=key+"_s", need_mtv=opt["need_mtv"]
                # )
                if model_ema is not None:
                    bests_n[key] = inference_set(
                        val_loaders[key],
