...
def get_spatial_and_temporal_and_mtvector_samples(
    video_path,
    sample_types,
    samplers,
    is_train=False,
    ...
    if video_path.endswith(".yuv"):
        ...
        while True:
            ...
            if cnt in np.unique(all_frame_inds):

                motion_field = torch.zeros([h, w, 2])
                for n in range(motion_vectors.shape[0]):
                    motion_field[motion_vectors[n, 6], motion_vectors[n, 5]] = torch.tensor(motion_vectors[n, 7:9][::-1].copy())
                mtvec_dict[cnt] = motion_field

            cnt += 1
        
        for stype in samplers:
            imgs = [frame_dict[idx] for idx in frame_inds[stype]]
            motion_fields = [mtvec_dict[idx] for idx in frame_inds[stype]]
            video[stype] = torch.stack(imgs, 0).permute(3, 0, 1, 2)
            mtvector[stype] = torch.stack(motion_fields, 0).permute(3, 0, 1, 2)
    ...
    for stype, sopt in sample_types.items():
        sampled_video[stype] = get_single_sample(video[stype], stype, 
                                                       **sopt)
        sampled_mv[stype] = get_single_sample(mtvector[stype], stype, 
                                                       **sopt)
    return sampled_video, frame_inds, sampled_mv
...
class FragmentSampleFrames:
    ...
    def __call__(self, total_frames, train=False, start_index=0):
        ...
        for i in range(frame_inds.shape[0]):
            if frame_inds[i] >= total_frames:
                frame_inds[i] = frame_inds[i-1] 
        return frame_inds.astype(np.int32)
...
class FusionDataset(torch.utils.data.Dataset):
    def __init__(self, opt, total_frames=192):  # todo: watch out!! new parameter 'total frame' for FlyEM!!
        ## opt is a dictionary that includes options for video sampling
        ...
        for stype, sopt in opt["sample_types"].items():
            ...
            # print(stype+" branch sampled frames:", self.samplers[stype](192, self.phase == "train"))
            print(stype + " branch sampled frames:", self.samplers[stype](total_frames, self.phase == "train"))
        ...
        if isinstance(self.ann_file, list):
            ...
            try:
                ...
                #if self.phase == "train":
                #    filenames = mat["train_video"][0]
                #    scores = mat["train_score"][0]
                #    for f in range(len(filenames)):
                #        filename = filenames[f][0]
                #        label = scores[f]
                #        filename = osp.join(self.data_prefix, filename)
                #        self.video_infos.append(dict(filename=filename, label=label))
                #if self.phase == "test":
                #    filenames = mat["val_video"][0]
                #    scores = mat["val_score"][0]
                #    for f in range(len(filenames)):
                #        filename = filenames[f][0]
                #        label = scores[f]
                #        filename = osp.join(self.data_prefix, filename)
    ...
    def __getitem__(self, index):
        ...
        # TODO: check here, normalize input?
...
class NewDataset(torch.utils.data.Dataset):
    def __init__(self, opt):
        ...
        self.phase = opt["phase"]
        self.crop = opt.get("random_crop", False)
        self.blurf_dir = opt["blur_feature"]
        self.mean = torch.FloatTensor([123.675, 116.28, 103.53])
        self.std = torch.FloatTensor([58.395, 57.12, 57.375])
        self.samplers = {}
        for stype, sopt in opt["sample_types"].items():
            if "t_frag" not in sopt:
                ...
                self.samplers[stype] = FragmentSampleFrames(sopt["clip_len"] // sopt["t_frag"], sopt["t_frag"], sopt["frame_interval"], sopt["num_clips"])
            print(stype+" branch sampled frames:", self.samplers[stype](192, self.phase == "train"))
            self.size_h = sopt["size_h"]
            self.size_w = sopt["size_w"]
    ...
    def __getitem__(self, index):
        ...
        video_length_read = 8
        extracted_frames = torch.zeros([video_length_read, 3, 720, 1280])   
        ...
        for i in range(video_length_read):
            ...
            read_frame = torchvision.transforms.ToTensor()(read_frame)
            extracted_frames[i] = read_frame

        extracted_frames = get_single_sample(extracted_frames)
        
        video_idx = path_name.split('/')[-1]
        feature_folder = os.path.join(self.blurf_dir, video_idx)
        blur_feature = torch.zeros([video_length_read, 320, 56, 56])
        for i in range(video_length_read):
            i_index = i   
            feature = np.load(os.path.join(feature_folder, 'blur_ feature_' + str(i_index) + '.npy'))
            feature = torch.from_numpy(feature)
            feature = feature.squeeze()
            blur_feature[i] = feature

        data["extracted_frames"] = extracted_frames
        data["blur_feature"] = blur_feature
        data["num_clips"] = {}
        for stype, sopt in self.sample_types.items():
            data["num_clips"][stype] = sopt["num_clips"]
        data["frame_inds"] = frame_inds
